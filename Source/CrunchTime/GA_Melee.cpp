...
void UGA_Melee::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
	...
	//implement a sprint with gameplay ability.
	//1, sprint take stamina, change max walk speed of the character movement component, show staimia in UI
	//2, sprint stops automatically, when no stamima left
	//3, stamina regenerate when not sprinting.
	//4, if holding down shift, but not moving, should not cost stamina.
